...
Mount SMB folder shared on a LAN:

[source,bash]
--
sudo mount -t cifs -o username=myusrname,uid=1000,gid=1000,vers=1.0 //192.168.1.10/public_folder /mnt/mysmbmount/
--
...
Install a package (`clang` as an example):

[source,bash]
--
sudo xbps-install -Su clang
--
...
Clean up `xbps` cache, orphaned packages and old kernels:
...
Find the package to which the given file belongs:

[source,bash]
--
xlocate -F /usr/share/nvim/runtime/indent.vim
--

List all packages that were manually installed:
